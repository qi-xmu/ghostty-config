...
<DialogModal title={t("modal.sharedConfig.title")} {onclose}>
    {#snippet icon()}
        <ShareIcon />
    {/snippet}

    <p class="share-modal-desc">{t("modal.sharedConfig.description")}</p>
    <div class="share-preview">
        ...
        {:else}
            {#if parseError}
                <div class="row p2">{t("modal.sharedConfig.parseError")}</div>
                <div class="row">&nbsp;</div>
            {/if}
    ...
    {#snippet footer()}
        <Button onclick={onclose}>{t("modal.dismiss")}</Button>
        <Button primary onclick={onimport}>{t("modal.sharedConfig.import")}</Button>
    {/snippet}
</DialogModal>
