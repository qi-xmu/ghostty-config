<script lang="ts">
    ...
    function remove() {
        ...
        selected = [];
        success(t("toast.keybindRemoved"));
    }
    ...
    function resetDefaults() {
        ...
        success(t("toast.keybindReset"));
    }

    async function requestResetDefaults() {
        const shouldReset = await confirm({
            title: t("modal.keybindReset.title"),
            message: t("modal.keybindReset.message"),
            confirmText: t("modal.keybindReset.confirm"),
    ...
    function handleSave(detail: string) {
        if (editorMode === "add") {
            ...
            success(t("toast.keybindUpdated"));
...
{:else}
    <Admonition>
        {@html t("keybinds.admonition").replace("<a>", '<a href="https://ghostty.org/docs/config/keybind">')}
    </Admonition>
<div class="expandable-list" in:fly={{y: 30, duration: 200}}>
    ...
    <div class="list-controls">
        <button onclick={addNew} type="button" title={t("keybinds.add")}>&plus;</button>
        <button onclick={remove} disabled={selected.length === 0} type="button" title={t("keybinds.remove")}>&ndash;</button>
        <button onclick={editSelected} disabled={selected.length !== 1} type="button" title={t("keybinds.edit")}>
        ...
        <button onclick={requestResetDefaults} type="button" title={t("keybinds.resetDefaults")}>
